...
import { Floret } from "./Decor";
import { LITE } from "../hooks";

// ───── origami bird ───────────────────────────────────────────────────────

function OrigamiBird({ size = 46 }: { size?: number }) {
  return (
    <svg viewBox="0 0 46 30" width={size} height={(size * 30) / 46} aria-hidden style={{ overflow: "visible" }}>
      ...
      <g className="ff-animate-wingFlap" style={{ transformOrigin: "20px 14px", transformBox: "fill-box" } as CSSProperties}>
...
function BirdsTransition({ dir }: { dir: 1 | -1 }) {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-80 overflow-hidden" style={{ transform: dir < 0 ? "scaleX(-1)" : undefined }}>
      {birds.map((b, i) => (
        <div
          key={i}
          className="ff-animate-birdCross absolute left-0 top-0"
          style={
            {
              "--by0": b.by0,
              "--bymid": b.bymid,
...
function Petal({ color }: { color: string }) {
  return (
    <svg viewBox="0 0 16 22" width="16" height="22" aria-hidden>
      <path d="M8 0 C 1 7, 1 16, 8 22 C 15 16, 15 7, 8 0 Z" fill={color} opacity="0.92" />
    </svg>
  );
}
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-80 overflow-hidden">
      {items.map((s, i) => (
        <div
          key={i}
          className="ff-animate-sweepAcross absolute left-0 top-0"
          style={
            {
              "--sx0": "-15vw",
              "--sx1": "120vw",
        ...
        >
          {s.flower ? (
            <svg viewBox="0 0 24 24" width="24" height="24" aria-hidden>
              <Floret cx={12} cy={12} r={10} petal={s.color} core="#A6F0E6" petals={6} />
            </svg>
          ) : (
            <Petal color={s.color} />
          )}
...
export function SectionTransition({ trigger }: { trigger: Trigger }) {
  ...
  return trigger.kind === "birds" ? <BirdsTransition key={trigger.key} dir={trigger.dir} /> : <PetalSweep key={trigger.key} dir={trigger.dir} />;
}
